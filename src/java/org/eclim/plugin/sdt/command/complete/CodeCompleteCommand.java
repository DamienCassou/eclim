  ...
  extends AbstractCommand
{
    ...
    throws Exception
  {
    ...
    CompletionDocument document = new CompletionDocument();
    ArrayList<CodeCompleteResult> results = new ArrayList<CodeCompleteResult>();
    for (IJavaCompletionProposal proposal : proposals){
      // force scala-ide to give us the completion via our dummy document
      ((ICompletionProposalExtension)proposal).apply(document, ' ', offset);

      String completion = document.getCompletion();
      ...
      results.add(new CodeCompleteResult(
            completion, description, shortDescription));
    }
  ...
  /* Couple hacky dummy objects to help get completion results */
  ...
  private class CompletionDocument
    extends Document
  {
    private String completion;

    public void replace(int pos, int length, String completion)
